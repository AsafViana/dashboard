...
export default function index() {
	...
	useEffect(() => {
		GetDadosLojas(Selecionado).then((dados) => {
			if(Selecionado === 'todos') {setExibirTelaTodos(true)}
			setDadosDasLojas(dados)
		})
	}, [DadosDasLojas])

	useEffect(() => {
		console.table(DadosDasLojas)
		console.log(ExibirTelaTodos)
	}, [Selecionado, ExibirTelaTodos])

	if (!!Lojas || !!DadosDasLojas) {
		if (Selecionado === 'todos' && ExibirTelaTodos) {
			return (
				<DashboardLayout>
					...
					<Stack spacing={{ xs: 1, sm: 2 }} direction='row' useFlexGap flexWrap='wrap' py={3} ustifyContent='space-around' alignItems='center'>
						{Object.keys(DadosDasLojas).map((key) => {
							if (key !== 'somatorio_total'){
								return (
									<Stack key={key} spacing={2} sx={{ justifyContent: 'center', width: '100%' }}>
										<Button variant='text' onClick={() => handleSelecionado(DadosDasLojas[key].cnpj)}>
